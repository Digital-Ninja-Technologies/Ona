...
// phone/website the LLM left null also gets a best-effort fill from a
// separate per-place Brave Search (see braveContactSearch) — heuristic,
// not guaranteed accurate, but never fabricated.
//
// Request body:  {
//   "message": string,
...
/**
 * Best-effort phone/website lookup for one place, used to fill in contact
 * details the LLM itself left null (it's told never to invent them). Purely
 * heuristic — a regex over Brave's search snippets for a phone number, and
 * the first result whose domain isn't an aggregator/social site for a
 * website — so it can occasionally pick an unrelated page; that's an
 * accepted trade-off for "usually right" over "almost always blank".
 */
async function braveContactSearch(
  name: string,
  address: string | null,
): Promise<{ phone: string | null; website: string | null }> {
  const query = address ? `${name} ${address} phone contact` : `${name} phone contact`;
  const results = await braveSearch(query, 3);
  if (results.length === 0) return { phone: null, website: null };
  ...
  return { phone, website: official?.url ?? null };
}
        ...
        if (Array.isArray(places)) {
          const withImages = await Promise.all(
            places.map(async (place) => {
              ...
              const [imageUrl, contact] = await Promise.all([
                braveImageSearch(place.name),
                // Only search for contact details the LLM didn't already
                // supply — avoids burning extra Brave quota when it's
                // already known.
                place.phone == null || place.website == null
                  ? braveContactSearch(place.name, address)
                  : Promise.resolve({ phone: null, website: null }),
              ]);
              return {
                ...place,
                imageUrl,
                phone: place.phone ?? contact.phone,
                website: place.website ?? contact.website,
              };
            }),
          );
